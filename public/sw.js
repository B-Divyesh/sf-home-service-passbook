const CACHE = 'home-service-passbook-v2';
...
self.addEventListener('install', (event) => {
  event.waitUntil(caches.open(CACHE).then(async (cache) => {
    await Promise.all(SHELL.map(async (path) => {
      ...
      const response = await fetch(`${path}${separator}precache=2`, { cache: 'reload' });
    ...
    if (page) {
      ...
      await Promise.all([...new Set(builtAssets)].map(async (path) => {
        const response = await fetch(`${path}?precache=2`, { cache: 'reload' });
        if (response.ok) await cache.put(path, response);
      }));
    }
